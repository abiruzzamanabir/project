...
<div class="row">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <a class="text-danger" href="{{ route('product.tag.status.update',$item->id) }}"><i
                                    ...
                                    @if (Auth::guard('admin')->user()->role->name == 'Admin')
                                    <a class="text-success" href="{{ route('product.tag.status.update',$item->id) }}"><i
                                ...
                                <td>
                                    ...
                                    <a class="btn btn-sm btn-warning" href="{{ route('products-tag.edit', $item->id) }}"><i
                                            class="fa fa-edit" aria-hidden="true"></i></a>
                                    @if ($form_type=='create')
                                    <form class="d-inline delete-form"
                                        action="{{ route('products-tag.destroy', $item->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
    ...
    <div class="col-md-4">
        ...
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">Add new products tag</h4>
            </div>
            @include('validate')
            <div class="card-body">
                <form action="{{ route('products-tag.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
        ...
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">Edit products Tag</h4>
            </div>
            @include('validate')
            <div class="card-body">
                <form action="{{ route('products-tag.update',$edit->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
